...
std::vector<OpWrapper> PreprocessPrefill(
  ...
  auto& transpose_output = tensor_pool.CreateNativeTensor(
      input_tensor.GetDataType(), input_tensor.GetQuantParams(),
...
bool TransformMHAToSHA(std::vector<OpWrapper>& ops, size_t start_id,
  ...
  const auto& add_after_matmulv_output =
      ops[start_id + 12 + id_offset].GetOutputTensor(0);
  ...
  for (int i = 0; i < num_heads; ++i) {
    ...
    auto& slice_output = tensor_pool.CreateNativeTensor(
        pattern_input_ptr->GetDataType(), pattern_input_ptr->GetQuantParams(),
        slice_output_dim);
    ...
    auto& mul_output = tensor_pool.CreateNativeTensor(
        mul_inputs[0].get().GetDataType(), mul_output_quant_param,
    ...
    auto& matmul1_output = tensor_pool.CreateNativeTensor(
        matmulk_cache_output.GetDataType(),
        matmulk_cache_output.GetQuantParams(), matmul1_output_dim);
    ...
    auto& matmul2_output = tensor_pool.CreateNativeTensor(
        matmulk_slice_output.GetDataType(),
        matmulk_slice_output.GetQuantParams(), matmul2_output_dim);
    ...
    auto& matmul1_v_output = tensor_pool.CreateNativeTensor(
        matmulv_cache_output.GetDataType(),
        matmulv_cache_output.GetQuantParams(), matmul1_v_output_dim);
    ...
    auto& matmul2_v_output = tensor_pool.CreateNativeTensor(
        matmulv_slice_output.GetDataType(),
        matmulv_slice_output.GetQuantParams(), matmul2_v_output_dim);
    ...
    auto& add_final_output = tensor_pool.CreateNativeTensor(
        add_after_matmulv_output.GetDataType(),
        add_after_matmulv_output.GetQuantParams(), add_final_output_dim);
  ...
  ops.insert(ops.begin() + start_id + 13 + id_offset,
             std::make_move_iterator(new_ops.begin()),
             std::make_move_iterator(new_ops.end()));
  QNN_LOG_INFO("Remove useless ops");
  // And then remove 0~14
  ops.erase(ops.begin() + start_id, ops.begin() + start_id + 13 + id_offset);
  return true;
}
}  // namespace qnn
